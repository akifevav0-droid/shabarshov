...
(function () {
  ...
  function close() {
    ...
    // прокрутку отпускаем сразу: иначе переход по пункту меню не успевает доехать
    document.documentElement.style.overflow = '';
    closeT = setTimeout(function () { el.hidden = true; closingEl = null; }, 580);
  ...
  // Меню на телефоне: та же механика, что у окон записи и цен
  var menu = document.getElementById('menu'), burger = document.querySelector('.nav__burger');
  if (menu && burger) {
    burger.addEventListener('click', function () {
      if (openEl === menu) { close(); return; }
      open(menu, menu.querySelector('.menu__links a'));
    });
    menu.querySelectorAll('.menu__links a').forEach(function (a) {
    ...
    document.addEventListener('click', function () { setTimeout(mark, 0); });
    document.addEventListener('keydown', function () { setTimeout(mark, 0); });
  }
  ...
  // Пересобираем при смене ширины: после поворота телефона копии карточек
  // иначе остаются на широком экране и прайс показывается трижды.
  (function () {
    var narrow = window.matchMedia('(max-width: 734px)');
    var built = [];
    ...
    function build() {
      document.querySelectorAll('#formaty .list, .reviews').forEach(function (box) {
        ...
        var stop = false;
        built.push({ box: box, items: items, off: function () { stop = true; } });
        ...
        requestAnimationFrame(function () { if (stop) return; box.style.scrollSnapType = 'none'; box.scrollLeft = center(n); box.style.scrollSnapType = ''; });
    ...
    function tear() {
      built.forEach(function (b) {
        b.off();
        while (b.box.firstChild) b.box.removeChild(b.box.firstChild);
        b.items.forEach(function (el) { b.box.appendChild(el); });
        b.box.style.scrollSnapType = '';
        b.box.scrollLeft = 0;
      });
      built = [];
    }
    ...
    var sync = function () {
      ...
      return function () {
        ...
        wait = setTimeout(function () {
          if (narrow.matches) { if (!built.length) build(); }
          else if (built.length) tear();
        }, 200);
      };
    }();

    if (narrow.matches) build();
    // Слушаем и медиазапрос, и обычное изменение размера: на части устройств
    // при повороте приходит только одно из двух.
